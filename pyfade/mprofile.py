...
def wavelet_MP_from_KDP(data: Union[pd.DataFrame, pd.Series], subseq_size: int, dimension: int, **kwargs) -> pd.DataFrame:
    ...
    if isinstance(data,pd.Series):
        ...
        MP = [None]*num_dim
        shutdowns = kwargs.get('shutdowns',None)
        shutdown = None

        for k in range(num_dim):

            if shutdowns is not None:
                shutdown = shutdowns[k]

            # Getting values
            MP[k] = wavelet_KDP(data.iloc[:,k],subseq_size,dimension,shutdown=shutdown, **kwargs)
            MP[k].rename(columns={MP[k].columns[0]:data.columns[k]},inplace=True)
...
def wavelet_KDP(data: Union[pd.DataFrame, pd.Series], subseq_size: int, dimension: int, **kwargs) -> pd.DataFrame:
    ...
    level = kwargs.get('level',dimension)
    on_signal = kwargs.get('on_signal',True)
...
def get_MP_from_wavelets(data: Union[pd.DataFrame, pd.Series], subseq_size: int, level: int = 1, on_signals: bool = True, create_plot: bool = False, plot_data: bool = False, **kwargs) -> Any:
    ...
    if on_signals:
        ...
        for lvl in range(level):
            ...
            MPs[lvl] = get_MP(cur_sig,subseq_size,normalize=False,**kwargs)[0]
        ...
        for lvl in range(level):
            ...
            MPs[lvl] = get_MP(cur_coef,subseq_size,normalize=False,**kwargs)[0]
        ...
        MPs[level] = get_MP(cur_coef,subseq_size,normalize=False,**kwargs)[0]
    ...
    if create_plot:
        ...
        fig_keys = ['style', 'linewidth', 'markersize','ylabel']
        fig_kwargs = {}
        for key in fig_keys:
            fig_kwargs[key] = kwargs.get(key,None)
        
        style = fig_kwargs.get('style',['r-','xb'])
        if not isinstance(style,list):
            style = [style,style]

        if plot_data:
            
            fig_kwargs['style'] = style[1]
            fig, axs = plot_multiple(data_plot, **fig_kwargs)

            fig_kwargs['style'] = style[0]
            fig_kwargs.pop('ylabel',None)
            plot_multiple(MPs,fig=fig,axs=axs,ylabel='MP',**fig_kwargs)
        else:
            fig_kwargs['style'] = style[0]
            fig, axs = plot_multiple(MPs,**fig_kwargs)
